...
def test_user_contract(to_session: TOSession,
	...
	try:
		...
		# Delete user after test execution to avoid redundancy.
		user_id = user_post_data.get("id")
		if to_session.delete_user_by_id(user_id=user_id) is None:
			logger.error("user returned by Traffic Ops is missing an 'id' property")
			pytest.fail("Response from delete request is empty, Failing test_user_contract")

		tenant_id = user_post_data.get("tenantId")
