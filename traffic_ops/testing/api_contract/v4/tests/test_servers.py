...
def test_server_contract(
	...
	try:
		...
		logger.info(f"Server Api response {first_server}")
		...
		keys = ["cachegroupId", "cdnId", "domainName", "physLocationId", "profileNames", "statusId", "typeId"]
		prereq_values = [server_post_data[key] for key in keys]
		get_values = [first_server[key] for key in keys]
		
		assert validate(instance=first_server, schema=server_response_template) == None
